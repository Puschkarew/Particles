...
let root = url.pathname.replace(/\/([^/]+\.html)?$/g, '');

// Remove /iframe if present
root = root.replace(/\/iframe/g, '');

/**
 * @type {string}
 */
// For standalone version, use current directory ('.') if root is empty or just '/'
// This ensures relative paths work correctly
export const rootPath = (root && root !== '/') ? root : '.';
