...
class ExampleLoader {
    ...
    async start({ engineUrl, fileNames }) {
        ...
        // Use default name if no match (for root path)
        const exampleName = match ? match[1] : 'gaussian-splatting_reveal';
        ...
        await Promise.all(fileNames.map(async (name) => {
            unorderedFiles[name] = await fetchFile(`/iframe/${exampleName}.${name}`);
        }));
